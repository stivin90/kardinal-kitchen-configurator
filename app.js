...
const STATE = {
  panX: 0, // px: horizontal pan offset
  wallMm: 3000,
...
function fmtMm(n){ return `${Math.max(0, Math.round(n))} мм`; }
...
function wireCanvasDrag(){
  ...
  const onDown = (ev) => {
    ...
    if(!hit){
      ...
      STATE.drag.panStartX = x;
      STATE.drag.panStartPanX = STATE.panX || 0;
  ...
  const onMove = (ev) => {
    if(STATE.drag.isPanning){
      const dx = x - STATE.drag.panStartX;
      STATE.panX = (STATE.drag.panStartPanX || 0) + dx;
      renderAll();
      e.preventDefault();
      return;
    }
...
function renderCanvas(){
  ...
  const freePx = Math.round(freeMm * scale);
  const usedPx = Math.round(used * scale);
  ...
  if(freeMm > 0){
    ...
    ctx.fillText(`Свободно: ${Math.round(freeMm)} мм`, freeX + 10, baseY-250);
  }
...
function computeLayout(scene){
  ...
  const wall = Math.max(1, STATE.wallMm);
  const scale = scene?.scale ?? (sceneW / wall);

  const usedMmAll = totalWidthMm();
  const contentW = Math.max(sceneW, Math.round(usedMmAll * scale));
  STATE.panX = clampPan(STATE.panX || 0, sceneW, contentW);
  ...
  const upperH = Math.min(140, Math.max(110, Math.round(tallH * 0.40)));
  const upperGap = Math.max(170, tallH - upperH); // distance from baseY to bottom of upper modules
  const baseH = Math.min(170, Math.max(130, Math.round(tallH * 0.50)));
  ...
  for(const it of STATE.items){
    ...
    if(row === "upper"){
      ...
      x = pad + Math.round(xUpperMm * scale) + (STATE.panX||0);
      xUpperMm += m.widthMm;
      bumpUpperCursorIfOverTall();
    } else {
      x = pad + Math.round(xLowerMm * scale) + (STATE.panX||0);
      xLowerMm += m.widthMm;
    }
...
function drawWallTicks(ctx, x, y, w, wallMm, scale){
  ...
  for(let mm=0; mm<=wallMm; mm+=minor){
    const px = x + Math.round(mm * scale);
...
// ---------- Pan along wall (left/right) ----------
function clampPan(panX, sceneW, contentW){
  if(contentW <= sceneW) return 0;
  const minX = -(contentW - sceneW);
  const maxX = 0;
  return Math.min(maxX, Math.max(minX, panX));
}
