...
function computeLayout(scene){
  ...
  const baseY = scene?.baseY ?? (pad + (cssH - pad*2)*0.72);
  ...
  for(const it of STATE.items){
    ...
    let h;
    if(m.kind === "tall") h = 350;
    else if(m.kind === "upper") h = 140;
    else h = 170;
    ...
    if(row === "upper"){
      ...
      x = pad + Math.round(xUpperMm * scale);
      xUpperMm += m.widthMm;
      bumpUpperCursorIfOverTall();
    } else {
      x = pad + Math.round(xLowerMm * scale);
      xLowerMm += m.widthMm;
    }
    ...
    let y;
    if(row === "upper"){
      const upperBottomY = baseY - 210; // зазор над столешницей
      y = upperBottomY - h;
    } else {
      y = baseY - h;
    }
